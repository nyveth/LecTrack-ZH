...
`BATCH_SIZE`, `WHISPER_MODEL_SIZE`, `WHISPER_DEVICE`, `WHISPER_COMPUTE_TYPE`,
...
The three `WHISPER_*` constants are the ones a different machine has to change.
`WHISPER_DEVICE` defaults to `cuda` and `WHISPER_COMPUTE_TYPE` to `int8`, which
is what a 3 GB card can hold. Without a usable GPU, set `WHISPER_DEVICE` to
`cpu` - transcription then runs at a small fraction of real-time speed, which is
why the deployed setup keeps it on the GPU machine. `WHISPER_MODEL_SIZE` trades
accuracy for VRAM; `small` is the largest that fits alongside the embedding
model on 3 GB.
...
Only the input directory has to be created. `transcribe_file()` and
`chunk_file()` create their own output directory from the path they are given,
so both the CLI and the worker are covered.
